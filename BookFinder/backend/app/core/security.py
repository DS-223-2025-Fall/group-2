...
def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    This function is used as a FastAPI dependency. It:
    - Retrieves the token from the request via OAuth2PasswordBearer.
    - Decodes the JWT using the application's secret key.
    - Returns basic user information from the token payload.
